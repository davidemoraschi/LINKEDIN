DROP PROCEDURE REQUEST_TOKEN;

CREATE OR REPLACE PROCEDURE          request_token (originalurl IN VARCHAR2 := 'about:blank', port IN VARCHAR2 := '0')
IS
	...
	oauth_callback 					VARCHAR2 (1000) := 'http://moraschi.eu:8081/sso/request_token_callback';
...
BEGIN
	...
	oauth_base_string :=
		utl_linkedin.base_string_callback (http_method
													 ...
													 ,oauth_timestamp
													 ,oauth_nonce);
	oauth_signature := utl_linkedin.signature (oauth_base_string, utl_linkedin.key_token (oauth_consumer_secret, NULL));
	...
	THEN
		...
		LOOP
			UTL_HTTP.get_header (http_resp, i, var_http_header_name, var_http_header_value);
	...
	INSERT INTO oauth_linkedin_parameters (oauth_token, oauth_token_secret, account)
	VALUES		(TRIM (oauth_request_token (2)), TRIM (oauth_request_token_secret (2)), TRIM (oauth_request_token (2)));
